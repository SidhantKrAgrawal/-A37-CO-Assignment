...
def TypeA(instrn,output,count):
    if len(instrn) != 4:
        sys.stdout.write("SyntaxError: Not all required paramters provided for Type A command in line no: ",count)
        quit()
    for i in range(1,4):
        if instrn[i] not in registers:
            sys.stdout.write("Syntax error: The given input does not have the type of instructon! in line no: ",count)
            quit()
    Bin=""
...
def TypeB(instrn,output,count):
    if len(instrn) != 3:
        sys.stdout.write("SyntaxError: Not all required paramters provided for Type B command in line no: ",count)
        quit()
    if instrn[2][0] != "$" and instrn[1][0] == "R":
        TypeC(instrn,output)
    elif instrn[1][0] != "R":
        sys.stdout.write("Syntax Error in line no: ",count)
        quit()
    else:
        x=int(instrn[-1][1:])
        if x<0 or x>255:
            sys.stdout.write("Syntax error: Illegal Immediate values in line no: ",count)
            quit()
        
        if instrn[1] not in registers:
            sys.stdout.write("Invalid register used in line no: ",count)
            quit()
    Bin = ""
...
def TypeC(instrn,output,count):
    if len(instrn) != 3:
        sys.stdout.write("SyntaxError: Not all required paramters provided for Type C command in line no: ",count)
        quit()
    Bin = ""
...
def TypeD(instrn,output,instructions,count):
    if len(instrn)!=3:
        sys.stdout.write("SyntaxError: Not all required parametrs provided for Type D command in line no: ",count)
        quit()
    if instrn[1] not in registers:
        sys.stdout.write("Invalid register used in line no: ",count)
        quit()    
    ...
    if instrn[-1] not in variables:
        sys.stdout.write("SytaxError: Misuse of variables in line no: ",count)
        quit()
...
def TypeE(instrn,output,instructions,count):
    if len(instrn)!=2:
        sys.stdout.write("SyntaxError : Wrong syntax used for instructions in line no: ",count)
        quit()
    Bin = ""
    ...
    if instrn[1] in labels:
        ...
        sys.stdout.write("ERROR: Memory address of Type E not defined in line no: ",count)
